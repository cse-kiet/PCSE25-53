...
│   ├── 7sem_dec_certificate.pdf      # Certificate for 7th semester signed by project guide and Dean CSE
│   ├── 8sem_may_certificate.pdf      # Certificate for 8th semester signed by project guide and Dean CSE
...
1.  **Image Input:** The user uploads an image via the Streamlit interface (`final_app.py` using `app/ui.py`).
2.  **Embedding Generation (User Image):** The uploaded image is processed by the CLIP model (loaded via `app/resources.py`) to generate a numerical embedding representing its visual features.
3.  **Similarity Search:** This embedding is compared against a pre-computed FAISS index (`data/aggregated_clip.index`) of embeddings for various tourist locations using logic in `app/image_search.py`. The most visually similar locations are retrieved.
    *   The `data/aggregated_clip.index`, `data/place_mapping.json` (maps location name to a representative image), and `data/places_order.json` (maps FAISS index ID to location name) are generated once by `embedding_gen.py` using images from the `places_images/50 most famous places in the world.zip/` directory.
4.  **User Customization:** The user selects a primary location from the suggestions and provides preferences like travel dates, interests, and search radius (handled in `app/ui.py`).
5.  **Geospatial Analysis & POI Enrichment:** The Google Maps API (interacted via `app/gmaps_api.py`) is used to geocode the selected location, find nearby attractions based on user preferences, and fetch details like opening hours, ratings, and travel times.
6.  **Itinerary Optimization:** The collected data is fed into Google OR-Tools (logic in `app/routing.py`), which solves a Vehicle Routing Problem with Time Windows (VRPTW) to create an optimized day-by-day itinerary.
7.  **Output Presentation:** The final itinerary is displayed to the user via `app/ui.py`, along with an interactive map (via Folium) showing routes and locations.
...
*   Run the `embedding_gen.py` script:
    ...
    ```
*   This script will create the `data/` directory (if it doesn't exist) and place the following files inside it (`aggregation_search/data/`):
    ...
    **Note:** The `embedding_gen.py` script expects the `places_images` directory to be located one level above the `aggregation_search` directory (i.e., in the project root). The script uses `base_folder = os.path.join("..", "places_images")`. The output data is now saved to the `data/` subdirectory within `aggregation_search/`.

### 2. Run the Streamlit Application

Once the embeddings and index are generated and located in the `aggregation_search/data/` directory, you can run the main application.
...
*   Run the `final_app.py` script using Streamlit:
    ...
    ```
*   This will start the Streamlit web server, and your default web browser should open to the application's URL (usually `http://localhost:8501`). The application will look for the index files in the `data/` subdirectory.

## Included Documents

The repository contains several supporting documents, primarily located within the `PCSE25-53/documents/` directory:

*   **`documents/7sem_dec_certificate.pdf`**: The certificate for 7th semester signed by project guide and Dean CSE.
*   **`documents/8sem_may_certificate.pdf`**: The certificate for 8th semester signed by project guide and Dean CSE.
...
Also, consider adding `aggregation_search/data/` to your `.gitignore` if you don't want to commit the generated index files (which can be large and are reproducible).